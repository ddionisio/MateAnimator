...
// NOTE: Didn't find in DOTween distribution
...
namespace MateAnimator {
    ...
    /// <para>To use it, call DOTween.To with the plugin parameter overload, passing it <c>AMPlugQuaternion.Plug()</c> as first parameter
    /// (do not use any of the other public AMPlugQuaternion methods):</para>
    /// <code>DOTween.To(AMPlugQuaternion.Plug(), ()=> myQuaternionProperty, x=> myQuaternionProperty = x, myQuaternionEndValue, duration);</code>
    /// </summary>
    public class AMPlugQuaternion : ABSTweenPlugin<Quaternion, Quaternion, NoOptions> {
        static AMPlugQuaternion _plug;
        /// <summary>
        /// Plug this plugin inside a DOTween.To call.
        /// <para>Example:</para>
        /// <code>DOTween.To(AMPlugQuaternion.Plug(), ()=> myQuaternionProperty, x=> myQuaternionProperty = x, myQuaternionEndValue, duration);</code>
        /// </summary>
        public static AMPlugQuaternion Plug() {
            if(_plug == null) _plug = new AMPlugQuaternion();
            return _plug;
        }

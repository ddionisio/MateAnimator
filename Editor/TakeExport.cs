...
namespace M8.Animator.Edit {
    public class TakeExport : EditorWindow {
        public static TakeExport window = null;
        ...
        void showItems() {
            ...
            if(didLoad) {
                if(gameObjs.Where(p => p != null && p.GetComponent<Animate>() != null).ToList().Count >= 1) {
                    ...
                    GUILayout.Label(new GUIContent("Animate (" + (take == null ? "All Takes" : take.name) + ")", EditorGUIUtility.ObjectContent(null, typeof(MonoBehaviour)).image));
                    GUILayout.EndVertical();
                    GUI.enabled = true;
                    GUILayout.EndHorizontal();
                }
                ...
                for(int i = 0; i < gameObjs.Count; i++) {
                    ...
                    if(gameObjs[i].GetComponent<Animate>() != null) continue;

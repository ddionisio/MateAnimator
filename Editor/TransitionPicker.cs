...
namespace M8.Animator.Edit {
    public class TransitionPicker : EditorWindow {
        ...
        public static int[] TransitionOrder = new int[]{
            (int)CameraSwitcherKey.Fade.None,
            (int)CameraSwitcherKey.Fade.CrossFade,
            (int)CameraSwitcherKey.Fade.Doors,
            (int)CameraSwitcherKey.Fade.VenetianBlinds,
            (int)CameraSwitcherKey.Fade.IrisBox,
            (int)CameraSwitcherKey.Fade.IrisRound,
            (int)CameraSwitcherKey.Fade.IrisShape,
            (int)CameraSwitcherKey.Fade.LinearWipe,
            (int)CameraSwitcherKey.Fade.ShapeWipe,
            (int)CameraSwitcherKey.Fade.RadialWipe,
            (int)CameraSwitcherKey.Fade.WedgeWipe

        };

        #region Declarations
        // static
        public static TransitionPicker window = null;
        public static bool justSet = false;
        public static CameraSwitcherKey key = null;
        ...
        void OnGUI() {
            ...
            GUI.color = TimelineWindow.getSkinTextureStyleState("properties_bg").textColor;
            ...
            if(GUI.Button(rectPlaybackButton, TimelineWindow.getSkinTextureStyleState("nav_skip_forward").background, GUI.skin.GetStyle("ButtonImage"))) {
                percent = 1f;
                updateValue();
            }
            ...
            if(isPlaying) playToggleTexture = TimelineWindow.getSkinTextureStyleState("nav_stop_white").background;
            else playToggleTexture = TimelineWindow.getSkinTextureStyleState("nav_play").background;
            if(GUI.Button(rectPlaybackButton, playToggleTexture, GUI.skin.GetStyle("ButtonImage"))) {
                isPlaying = !isPlaying;
            }
            rectPlaybackButton.x -= (18f + 2f);
            // first frame button
            if(GUI.Button(rectPlaybackButton, TimelineWindow.getSkinTextureStyleState("nav_skip_back").background, GUI.skin.GetStyle("ButtonImage"))) {
                percent = 0f;
                updateValue();
            }
            #endregion
            ...
            if(GUI.Button(rectClearButton, TimelineWindow.getSkinTextureStyleState("x").background, GUI.skin.GetStyle("ButtonImage"))) {
                searchString = "";
            }
            GUILayout.EndHorizontal();
            GUILayout.FlexibleSpace();
        ...
        #region Get / Set
        public static void setValues(CameraSwitcherKey _key, Track _track) {
            justSet = true;
            key = _key;
            track = _track;
        ...
        public static void setDefaultParametersForKey(ref CameraSwitcherKey cKey) {
        ...
        public int getSelectedTransitionIndex(List<string> filteredNames) {
            ...
            int index = filteredNames.IndexOf(TransitionNamesDict[selectedTransition]);
            return index;
        }

        public string getTransitionName(int index) {
            if(index >= TransitionNamesDict.Length) index = 0;
            return TransitionNamesDict[index];
        }
        ...
        public bool setSelectedTransition(List<string> filteredNames, int index) {
            ...
            int _index = TransitionOrder[System.Array.IndexOf(TransitionNames, filteredNames[index])];
        ...
        public void reloadAnimate() {
            ...
            if(aData != null) {
                ...
                foreach(Track _track in take.trackValues) {
                    if(track == _track) {
                        track = _track;
                        foreach(CameraSwitcherKey _key in track.keys) {
                            if(key == _key) {
                                key = _key;
                                shouldClose = false;
                            }
        ...
        public List<string> filterWithSearchString(string[] lsNames, string _searchString) {
            if(_searchString == null || _searchString == "") return new List<string>(lsNames);

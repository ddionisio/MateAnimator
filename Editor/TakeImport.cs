...
namespace M8.Animator.Edit {
    public class TakeImport : EditorWindow {
        ...
        void saveChanges() {
            if(!TimelineWindow.window) return;
        ...
        public static void openAdditiveAndDeDupe(string scenePath) {
            ...
            // merge Animate
            List<AnimateEditControl> origAnimatorData = new List<AnimateEditControl>();
            List<AnimateEditControl> newAnimatorData = new List<AnimateEditControl>();
            // get animator data
            foreach(GameObject go in origGOs) {
                Animate _temp = getAnimatorData(go);
                if(_temp != null) origAnimatorData.Add(new AnimateEditControl(_temp));
            }
            foreach(GameObject go in newGOs) {
                Animate __temp = getAnimatorData(go);
                if(__temp != null) newAnimatorData.Add(new AnimateEditControl(__temp));
            }
            int numTakes = 0;
            foreach(AnimateEditControl _a in newAnimatorData) {
        ...
        public static Animate getAnimatorData(GameObject go) {
            return go.GetComponent<Animate>();
        }
    }
}
